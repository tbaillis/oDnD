...
export class OpenAIDMAgent {
  ...
  // Generic helper to call MCP tools via DM server HTTP API
  private async callTool(tool: string, parameters: any) {
    try {
      const res = await fetch(`${DM_API_BASE_URL}/tool`, {
        method: 'POST',
  ...
  public async moveParty(positionsOrDelta: any) {
    // Accept either { positions: [...] } or { dx, dy }
    return this.callTool('move_party', positionsOrDelta);
  }

  // Combat and utility tool wrappers
